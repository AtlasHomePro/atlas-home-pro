    ...
    bar.innerHTML = [
      '<div class="us-inner">',
        '<div class="us-copy">',
          ...
          '<h2 class="us-headline">A letter from the field, every&nbsp;Friday.</h2>',
        '</div>',
        '<form class="us-form" name="atlas-dispatch" method="POST" data-netlify="true" action="/thanks/" onsubmit="return window.atlasUnivSub(this, event)">',
          ...
          '<label class="us-label" for="us-email">Email address</label>',
          '<input type="email" id="us-email" name="email" required placeholder="name@domain.com" autocomplete="email" aria-label="Email address" />',
          '<div class="us-row">',
            '<p class="us-fine">No ads. No funnels. One short note from David on what is moving in LA.</p>',
            '<button type="submit">Subscribe</button>',
          '</div>',
          '<div class="us-success"><span>You are on the list. First dispatch lands Friday.</span></div>',
        '</form>',
      '</div>'
    ].join('');
